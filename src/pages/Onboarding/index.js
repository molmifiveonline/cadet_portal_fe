...
const Onboarding = () => {
  const [rows, setRows] = useState([]); const [search,setSearch]=useState(''); const [status,setStatus]=useState(''); const [loading,setLoading]=useState(true); const [saving,setSaving]=useState(null);
  ...
  useEffect(() => { load(); }, [load]);
  const filtered = useMemo(() => rows.filter((row) => (!status || row.status === status) && (!search || `${row.name_as_in_indos_cert} ${row.cadet_unique_id} ${row.vessel_name}`.toLowerCase().includes(search.toLowerCase()))), [rows,search,status]);
  ...
  return <div className="py-6"><PageHeader title="Cadet Onboarding" subtitle="Checklist for finalized CTV Assigned candidates" icon={ClipboardCheck} />
